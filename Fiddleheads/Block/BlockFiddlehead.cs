    ...
    public class BlockFiddlehead : BlockCrop
    {
        ...
        */
        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "fiddleheadBlockInteractions", () =>
            {
                return new WorldInteraction[] {
                    new WorldInteraction()
                    {
                        ActionLangCode = "blockhelp-fiddlehead-harvest",
        ...
        public override ItemStack[] GetDrops(IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1f)
        {
            ...
            if (befarmland == null)
            {
                ...
                foreach (var drop in drops)
                {
                    ...
                    {
                        drop.StackSize = GameMath.RoundRandom(world.Rand, wildProps.WildFernDropMul * drop.StackSize);
                    }
        ...
        {
            // api.Logger.Notification($"Broken fiddlehead at {pos}");
        ...
        {
            // api.Logger.Notification($"Try to spawn BEFiddlehead at {pos}");
            ...
            if (!block.HasBehavior<BlockBehaviorFiddleheadHost>())
            {
                rootPos.Down();
                block = blockAccessor.GetBlock(rootPos);
                if (!block.HasBehavior<BlockBehaviorFiddleheadHost>()) return false;
            }

            blockAccessor.SpawnBlockEntity("BEFiddlehead", rootPos);
            (blockAccessor.GetBlockEntity(rootPos) as BlockEntityFiddlehead)?.OnGenerated(blockAccessor, worldGenRand, this);
            // api.Logger.Notification($"Placed BEFiddlehead at {pos}");
            return true;
        }
